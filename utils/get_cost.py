...
def cal_trans(A, X=None):
    ...
    if X is None:
        X = torch.ones((n, 1)).to(torch.float32)
...
def get_sep_rwr(T1, T2, H, beta, sepRwrIter):
    ...
    e1 = torch.zeros((n1, num_anchors)).to(torch.float32)
    e2 = torch.zeros((n2, num_anchors)).to(torch.float32)
    e1[(anchors1, torch.arange(num_anchors))] = 1
    e2[(anchors2, torch.arange(num_anchors))] = 1

    r1 = torch.zeros((n1, num_anchors)).to(torch.float32)
    r2 = torch.zeros((n2, num_anchors)).to(torch.float32)
...
def get_cross_cost(X1, X2, H):
    ...
    if X1_zero_pos[0].shape[0] != 0:
        X1[X1_zero_pos] = torch.ones(d).to(torch.float32)
    if X2_zero_pos[0].shape[0] != 0:
        X2[X2_zero_pos] = torch.ones(d).to(torch.float32)
...
def get_intra_cost(X):
    ...
    if X_zero_pos[0].shape[0] != 0:
        X[X_zero_pos] = torch.ones(d).to(torch.float32)
    X = X / torch.linalg.norm(X, dim=1, ord=2, keepdim=True)
...
def get_prod_rwr(L1, L2, nodeCost, H, beta, gamma, prodRwrIter):
    ...
    crossCost = torch.zeros((nx, ny)).to(torch.float32)
